...
"use strict";var SLIDER=new Swiper(".page",{wrapperClass:"page__wrapper",slideClass:"page__screen",direction:checkSliderOrientation(),mousewheel:{sensitivity:1},keyboard:{enabled:!0,onlyInViewport:!0,pageUpDown:!0},speed:800,pagination:{el:".page__pagination",type:"bullets",clickable:!1,bulletClass:"page__bullet",bulletActiveClass:"page__bullet_active"},navigation:{nextEl:".scroll-indicator-next",prevEl:".scroll-indicator-prew"}}),numOfSliders=document.querySelectorAll(".page__screen").length,progressPointsPerSlide=1/(numOfSliders-1),pointerCircles=document.querySelectorAll(".cls-3"),pointerLine=document.querySelectorAll(".cls-5"),pointerHead=document.querySelectorAll(".cls-6");function addActiveClass(e){e.forEach(function(e,n){e.classList.contains("active")||e.classList.add("active")})}var vid=document.getElementsByClassName("video__item");function play(e){vid[e].play()}function addClass(e,n){e.classList.add(n)}function cleanClass(e,n){for(var i=0;i<e.length;i++)e[i].classList.contains(n)&&e[i].classList.remove(n)}function removeHiddenProp(e){e.hidden=!1}function addHiddenPropForAll(){for(var e=0;e<vid.length;e++)vid[e].hidden=!0}function windowLoad(){addActiveClass(pointerCircles),addActiveClass(pointerLine),addActiveClass(pointerHead)}function stopVideoForAll(){for(var e=0;e<vid.length;e++)vid[e].pause(),vid[e].currentTime=0}function checkSliderOrientation(){var e=parseInt(document.documentElement.clientWidth),e=parseInt(document.documentElement.clientHeight)<e?"vertical":"horizontal";return e}window.addEventListener("load",windowLoad),SLIDER.on("realIndexChange",function(){console.log("Событие realIndexChange. Номер слайда: ".concat(SLIDER.activeIndex,". Предыдущий слайд: ").concat(SLIDER.previousIndex)),addHiddenPropForAll(),removeHiddenProp(vid[SLIDER.activeIndex]),0<SLIDER.activeIndex&&removeHiddenProp(vid[SLIDER.activeIndex-1]),cleanClass(vid,"displayed"),addClass(vid[SLIDER.activeIndex],"displayed"),stopVideoForAll(),0<SLIDER.activeIndex&&(play(SLIDER.activeIndex),console.log("Запуск плеера в кадре: ".concat(SLIDER.activeIndex)))}),SLIDER.on("progress",function(){0<=SLIDER.progress&&SLIDER.progress<=1&&(console.log("Событие progress: ".concat(SLIDER.progress)),!1===vid[SLIDER.activeIndex].ended&&0!=SLIDER.activeIndex&&(console.log("Перемотка при незавершенном видео в кадре: ".concat(SLIDER.activeIndex,". Время: ").concat(vid[SLIDER.activeIndex].currentTime)),vid[SLIDER.activeIndex].pause()),SLIDER.progress===progressPointsPerSlide*SLIDER.activeIndex&&(console.log("Событие Перемотка к последнему кадру слайда ".concat(SLIDER.activeIndex)),vid[SLIDER.activeIndex].currentTime<vid[SLIDER.activeIndex].duration&&0!=SLIDER.activeIndex&&vid[SLIDER.activeIndex].play()))}),SLIDER.on("beforeResize",function(){SLIDER.changeDirection(checkSliderOrientation(),!0)});
